...
<table>
    <c:if test="${(not empty resultSetName) and (empty resultNumName)}">
    <%-- ヘッダ行 --%>
    ...
    <c:if test="${(empty resultSetName) and (not empty resultNumName)}">
    <%-- ヘッダ行 --%>
    <jsp:invoke fragment="headerRowFragment" />
    <%-- ボディ行 --%>
    <n:set var="resultNum" name="${resultNumName}" scope="page" bySingleValue="false" />
    <c:forEach begin="0" end="${resultNum - 1}" varStatus="nablarch_status">
        <n:set var="${varStatusName}" value="${nablarch_status}" />
        <c:if test="${(nablarch_status.count % 2) != 0}">
            <n:set var="${varOddEvenName}" value="${oddValue}" />
        </c:if>
