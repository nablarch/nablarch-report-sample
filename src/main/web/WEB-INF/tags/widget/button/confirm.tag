...
--%>
<%@tag pageEncoding="UTF-8" description="確定ボタンを出力するウィジェット" %>
<%@ taglib prefix="button" tagdir="/WEB-INF/tags/widget/button" %>

<%-- ============================ 属性定義 =============================== --%>
<%@ attribute name="label"      description="ボタンに表示するラベルを設定する。省略した場合は、「確定」" rtexprvalue="true" %>
...
<%@ attribute name="allowDoubleSubmission" description="2重サブミットを許可するか否か。falseを指定した場合は許可しない。デフォルトはfalse" rtexprvalue="true" %>
...
<button:base
    ...
    allowDoubleSubmission="${(empty allowDoubleSubmission) ? false : allowDoubleSubmission}">
