...
<%@tag pageEncoding="UTF-8" description="テキストエリア項目を出力するウィジェット" %>
...
<%---------------------- 個別属性 ----------------------%>
<%@ attribute name="rows"      description="表示行数(デフォルト:4)" rtexprvalue="true" %>
<%---------------------- 属性定義（設計書） ----------------------%>
<%@ attribute name="dataFrom" description="表示するデータの取得元（画面項目定義に記載する、「表示情報取得元」.「表示項目名」の形式で設定する）" %>
<%@ attribute name="comment"    description="このテキストエリアについての備考（画面項目定義の項目定義一覧で、備考欄に表示される）" %>
...
<field:inputbase
  ...
  required   = "${required}"
  hint       = "${hint}"
  fieldClass = "textarea ${disabled ? 'disabled' : ''}"
  titleSize  = "${titleSize}"
  inputSize  = "${inputSize}">
